...
<h1><img src="<?=base_url('assets/imgs/shop-cart-add-icon.png')?>" class="header-img" style="margin-top:-3px;"> Add Pre launch</h1>
<hr>
<?php
$timeNow = time();
...
foreach ($languages as $language) {
  ...
  <div class="locale-container locale-container-<?=$language->abbr?>" <?=$language->abbr == MY_DEFAULT_LANGUAGE_ABBR ? 'style="display:block;"' : ''?>>
    ...
    <div class="form-group">
      <label>Title </label>
      <input type="text" name="title[]" required value="<?=$trans_load != null && isset($trans_load[$language->abbr]['title']) ? $trans_load[$language->abbr]['title'] : ''?>" class="form-control">
    </div>
    ...
    <div class="form-group">
      ...
      <textarea name="description[]" required id="description<?=$i?>" rows="50" class="form-control"><?=$trans_load != null && isset($trans_load[$language->abbr]['description']) ? $trans_load[$language->abbr]['description'] : ''?>
</textarea>
      <script>
...
$query = $this->db->query('SELECT translations_first.*, (SELECT name FROM shop_attribute_translations WHERE for_id = sub_for AND abbr = translations_first.abbr) as sub_is, shop_attribute.position FROM shop_attribute_translations as translations_first INNER JOIN shop_attribute ON shop_attribute.id = translations_first.for_id WHERE shop_attribute.sub_for = ' . $value['id'] . ' ORDER BY position ASC');
	...
	foreach ($result as $key => $values) {
		$valueid = '';
		if ($id != 0 || $id != '') {
			$query = $this->db->query('SELECT valueid FROM product_attribute WHERE preproductid = ' . $id . ' AND keyid = ' . $value['id']);
			$result = $query->row();
			$valueid = $result->valueid;
		}

		?>
              <option <?php if (isset($valueid) && $valueid == $values['id']) {echo 'selected="selected"';}?> value="<?=$values['id']?>">
              <?=$values['name']?>
              </option>
              <?php }?>
            </select></td>
...
<option value="0" <?= isset($_POST['in_slider']) && $_POST['in_slider'] == 0 || !isset($_POST['in_slider']) ? 'selected' : '' ?>>No</option>
  ...
  <div class="form-group for-shop">
    ...
    <select required class="selectpicker form-control show-tick show-menu-arrow" name="product_type">
      <option value="">Select Product Type</option>
      <option value="box" <?=isset($_POST['product_type']) && $_POST['product_type'] == 'box' ? 'selected' : ''?>>Box</option>
      <option value="loose" <?=isset($_POST['product_type']) && $_POST['product_type'] == 'loose' ? 'selected' : ''?>>Loose</option>
    </select>
  </div>
  <div class="form-group for-shop">
    <label>Pcs</label>
    <input required type="text" placeholder="number" name="product_pcs" value="<?=@$_POST['product_pcs']?>" class="form-control" id="quantity">
  </div>
  <div class="form-group for-shop">
  ...
  <div class="form-group for-shop">
    <label>Min Qty</label>
    <input required type="text" placeholder="number" name="min_qty" value="<?=@$_POST['min_qty']?>" class="form-control" id="quantity">
  </div>
  <div class="form-group for-shop">
