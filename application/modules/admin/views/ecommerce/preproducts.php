...
?>
    <h1><img src="<?=base_url('assets/imgs/products-img.png')?>" class="header-img" style="margin-top:-2px;">  Pre launch</h1>
    <hr>
    <a href="<?=base_url('admin/prepublish');?>" class="btn btn-primary btn-xs pull-right" style="margin-bottom:10px;"><b>+</b> Add Pre launch</a>
    <div class="row">
        <div class="col-xs-12">
...
foreach ($products as $row) {
		...
		?>
                                ...
                                <tr>
                                    <td>
                                        <a href="<?=base_url('admin/likedislikepreproimg/' . $row->id);?>">
...
$this->db->where('productid', $row->id);
		...
		?>
                                    <a href="<?=base_url('admin/likedislike/' . $row->id . '?status=1');?>" class="btn btn-primary">
                                    <i class="fa fa-thumbs-up" style="font-size: 18px;"></i>
                                    <span class="badge badge-light"><?=$this->db->count_all_results('likepreproduct');?></span>
                                    </a>
                                    </td>
                                    <td>
                                    <?php
$this->db->where('productid', $row->id);
		...
		?>
                                    <a href="<?=base_url('admin/likedislike/' . $row->id . '?status=0');?>" class="btn btn-primary">
		...
		<td><?= $row->position ?></td><?php */?>
                                    <td>
                                        <div class="pull-right">
                                            <a href="<?=base_url('admin/prepublish/' . $row->id)?>" class="btn btn-info">Edit</a>
                                            <a href="<?=base_url('admin/preproducts?delete=' . $row->id)?>"  class="btn btn-danger confirm-delete">Delete</a>
                                        </div>
                                    </td>
                                </tr>
                                <?php
