<h1><img src="<?= base_url('assets/imgs/admin-user.png') ?>" class="header-img" style="margin-top:-3px;"> Edit Attributes</h1>
<hr>
<?php
$timeNow = time();
  ...
  <div class="col-md-4">
    <div class="panel panel-default">
      <div class="panel-heading">
        <h3 class="panel-title"><i class="fa fa-user"></i> Attributes Details</h3>
      </div>
      <form method="POST" action="" enctype="multipart/form-data">
          <input type="hidden" name="attributes_id" value="<?= $attributes_result['attributes_id']; ?>">
        <table class="table">
          <tbody>
          <tr>
              ...
              <td><input type="text" required id="title" name="title" placeholder="Title" value="<?= $attributes_result['title'] ?>" class="form-control"></td>
            </tr>
            <tr>
              ...
              <td><input type="text" required id="sort_order" name="sort_order" placeholder="Sort order" value="<?= $attributes_result['sort_order'] ?>" class="form-control"></td>
            </tr> 
            <tr>
              ...
              <td><select class="selectpicker form-control" data-live-search="true" name="refAttributes_group_id">  
                  <!-- <option value="">Select Attributes group</option> -->
                  <?php 
                  if(!empty($attributes_group_result)){
                  foreach($attributes_group_result as $row){ ?>
                  <option value="<?= $row['attributesgroup_id']; ?>" <?= $attributes_result['refAttributes_group_id'] == $row['attributesgroup_id'] ? 'selected' : '' ?>><?= $row['title'] ?></option>
                  <?php }} ?>
                </select></td>
            </tr>        
            <tr>
              ...
              <td><select class="form-control" name="status">
                  ...
                  <option value="0" <?= $attributes_result['status'] == '0' ? 'selected' : '' ?>>Inactive</option>
                  <option value="1" <?= $attributes_result['status'] == '1' ? 'selected' : '' ?>>Active</option>
                </select></td>
            </tr>
            <tr>
              <td><strong></strong></td>
