	...
	public function index($page = 0, $id = 0) {
		...
		if (isset($_POST['update'])) {
			if (!empty($_FILES['sliderfile']['name'])) {
				...
				$_POST['image'] = $_POST['slideroldimg'];
			}
			if (isset($_GET['to_lang'])) {
				$id = 0;
			}
			$this->Slider_model->setSlider($_POST, $_POST['updateid']);
			$this->session->set_flashdata('result_publish', 'Slider Successfully Updated!');
			if ($id == 0) {
				$this->saveHistory('Success published Slider');
			} else {
				$this->saveHistory('Success updated Slider');
		...
		$data['userlist'] = $this->Slider_model->getSlider($this->num_rows, $page, $search_title, $orderby, $category, $vendor);
		if (isset($_GET['edit'])) {
			$data['oneslider'] = $this->Slider_model->getoneSlider($_GET['edit']);
		}
