...
class Viewproduct extends ADMIN_Controller {

	private $num_rows = 10;

	public function __construct() {
		parent::__construct();
		$this->load->model(array('Viewproduct_model', 'Languages_model', 'Categories_model'));
	}
	...
	public function index($page = 0) {
		...
		if (isset($_GET['delete'])) {
			$this->Viewproduct_model->deleteProduct($_GET['delete']);
		...
		$rowscount = $this->Viewproduct_model->productsCount($search_title, $category);
		$data['userlist'] = $this->Viewproduct_model->getproducts($this->num_rows, $page, $search_title, $orderby, $category, $vendor, $status);
		$data['productinfo'] = $this->Viewproduct_model->getproductinfo($page);
		$data['links_pagination'] = pagination('admin/likedislike', $rowscount, $this->num_rows, 3);
		$data['num_shop_art'] = $this->Viewproduct_model->numShopproducts();
		$data['languages'] = $this->Languages_model->getLanguages();
		$data['shop_categories'] = $this->Categories_model->getShopCategories(null, null, 2);
		$this->saveHistory('Go to products');
		$this->load->view('_parts/header', $head);
		$this->load->view('user/viewproduct', $data);
	...
	public function getProductInfo($id, $noLoginCheck = false) {
		...
		*/
		...
		return $this->Viewproduct_model->getOneProduct($id);
	}
	...
	*/
	...
	public function productStatusChange() {
		$this->login_check();
		$result = $this->Viewproduct_model->productStatusChange($_POST['id'], $_POST['to_status']);
		if ($result == true) {
			echo 1;
		} else {
			echo 0;
