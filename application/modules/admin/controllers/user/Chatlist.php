...
class Chatlist extends ADMIN_Controller {
	...
	public function __construct() {
		parent::__construct();
		$this->load->model(array('Chat_model', 'Languages_model', 'Categories_model'));
	}
	...
	public function index($page = 0) {
		...
		$rowscount = $this->Chat_model->productsCount($search_title, $orderby, $category);
		$data['userlist'] = $this->Chat_model->getproducts($this->num_rows, $page, $search_title, $orderby, $category, $vendor);
		$data['links_pagination'] = pagination('admin/chatlist', $rowscount, $this->num_rows, 3);
		$data['num_shop_art'] = $this->Chat_model->numShopproducts();
		$data['languages'] = $this->Languages_model->getLanguages();
		$data['shop_categories'] = $this->Categories_model->getShopCategories(null, null, 2);
		$this->saveHistory('Go to user');
		$this->load->view('_parts/header', $head);
		$this->load->view('user/chatlist', $data);
	...
	public function getProductInfo($id, $noLoginCheck = false) {
		...
		*/
		...
		return $this->Chat_model->getOneProduct($id);
	}
	...
	*/
	...
	public function productStatusChange() {
		$this->login_check();
		$result = $this->Chat_model->productStatusChange($_POST['id'], $_POST['to_status']);
		if ($result == true) {
			echo 1;
		} else {
			echo 0;
