...
class Likedislike extends ADMIN_Controller
{
    ...
    {
        parent::__construct();
        $this->load->model(array('Likedislike_model', 'Languages_model', 'Categories_model'));
    }
    ...
    public function index($page = 0)
    {
        ...
        if (isset($_GET['delete'])) {
            $this->Likedislike_model->deleteProduct($_GET['delete']);
        ...
        $rowscount = $this->Likedislike_model->productsCount($search_title, $category);
        $data['userlist'] = $this->Likedislike_model->getproducts($this->num_rows, $page, $search_title, $orderby, $category, $vendor, $status);
		$data['productinfo'] = $this->Likedislike_model->getproductinfo($page);
        $data['links_pagination'] = pagination('admin/likedislike', $rowscount, $this->num_rows, 3);
        $data['num_shop_art'] = $this->Likedislike_model->numShopproducts();
        $data['languages'] = $this->Languages_model->getLanguages();
        $data['shop_categories'] = $this->Categories_model->getShopCategories(null, null, 2);
        $this->saveHistory('Go to products');
        $this->load->view('_parts/header', $head);
        $this->load->view('user/likedislike', $data);
    ...
    public function getProductInfo($id, $noLoginCheck = false)
    {
        ...
        return $this->Likedislike_model->getOneProduct($id);
    }
    ...
    {
        $this->login_check();
        $result = $this->Likedislike_model->productStatusChange($_POST['id'], $_POST['to_status']);
        if ($result == true) {
            echo 1;
        } else {
            echo 0;
