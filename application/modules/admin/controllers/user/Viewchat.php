...
class Viewchat extends ADMIN_Controller {
	...
	public function index($id = 0) {
		...
		$trans_load = null;
		if ($id > 0 && $_POST == null) {
			$_POST = $this->Chat_model->getOneProduct($id);
			$_POST['Addresslist'] = $this->Chat_model->getuseraddress($id);
			$_POST['Chatcomment'] = $this->Chat_model->chatcomment($id);
			$trans_load = $this->Chat_model->getTranslations($id);
		}
		if (isset($_POST['submit'])) {
			...
			$this->Chat_model->setProduct($_POST, $id);
			if ($id == 0) {
			...
			if (isset($_SESSION['filter']) && $id > 0) {
				...
				redirect(base_url('admin/chatlist?' . $get));
			} else {
		...
		if (isset($_POST['sendmessagechat']) && isset($_POST['commentmsg'])) {
			...
			$this->Chat_model->sendordersmsg($id);
			redirect('admin/viewchat/' . $id);
		}
		$data = array();
		$head = array();
		...
		$this->load->view('user/viewchat', $data);
	...
	*/
	...
	public function chatfunction() {
		$chatcomment = $this->Chat_model->chatcomment($_POST['user_id']);
		// echo '<pre>';
		// print_r($chatcomment);
		$data = '';
	...
	public function chatsendfunction() {
		...
		$this->db->where('id =',$this->input->post("receiver_id"));		
