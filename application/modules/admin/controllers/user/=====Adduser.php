	...
	*/
	...
	public function addbulkuser() {
		if (isset($_POST['submit'])) {
			...
			if ($List) {
				$this->session->set_flashdata('result_publish', "Some Users Mobile Already exits In System! $List");
			}
			$this->session->set_flashdata('result_publish', 'Customers Imported Successfully');
			redirect('admin/listuser');
			// redirect('admin/' . $this->module . '/showstaff?businessid=' . $businessid);
		}
		// echo "manish";
		// echo "bulkuser";exit;
		// $this->login_check();
