...
class Preproducts extends ADMIN_Controller {

	private $num_rows = 10;

	public function __construct() {
		parent::__construct();
		$this->load->model(array('Preproducts_model', 'Languages_model', 'Categories_model'));
	}
	...
	public function index($page = 0) {
		...
		if (isset($_GET['delete'])) {
			if (!in_array('deleteprelaunch', $adminid)) {redirect('admin');}
			$this->Preproducts_model->deleteProduct($_GET['delete']);
		...
		$rowscount = $this->Preproducts_model->productsCount($search_title, $category);
		$data['products'] = $this->Preproducts_model->getproducts($this->num_rows, $page, $search_title, $orderby, $category, $vendor);
		$data['links_pagination'] = pagination('admin/preproducts', $rowscount, $this->num_rows, 3);
		$data['num_shop_art'] = $this->Preproducts_model->numShopproducts();
		$data['languages'] = $this->Languages_model->getLanguages();
		$data['shop_categories'] = $this->Categories_model->getShopCategories(null, null, 2);
		$this->saveHistory('Go to products');
		$this->load->view('_parts/header', $head);
		$this->load->view('ecommerce/preproducts', $data);
	...
	public function getProductInfo($id, $noLoginCheck = false) {
		...
		*/
		...
		return $this->Preproducts_model->getOneProduct($id);
	}
	...
	*/
	...
	public function productStatusChange() {
		$this->login_check();
		$result = $this->Preproducts_model->productStatusChange($_POST['id'], $_POST['to_status']);
		if ($result == true) {
			echo 1;
		} else {
			echo 0;
