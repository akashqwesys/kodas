...
class Likedislikeproimg_model extends CI_Model {
	...
	public function getproductinfo($id) {
		...
		$this->db->limit(1);
		$query = $this->db->select('products.image as product_image, products.folder as imgfolder')->get('products');
		...
		if (isset($singalimg) && $singalimg != '') {
			...
			$querylike = $this->db->count_all_results('likeproductimg');
			$nid = 1;
			...
			$querylikedi = $this->db->count_all_results('likeproductimg');
		...
		foreach ($multiimgarray as $key => $value) {
			...
			$this->db->where('likeproductimg.imgname', $without_extension);
			$this->db->where('likestatus', '0');
			$querydislike = $this->db->count_all_results('likeproductimg');
			$this->db->where('likeproductimg.imgname', $without_extension);
			$this->db->where('likestatus', '1');
			$querylike = $this->db->count_all_results('likeproductimg');
			$likeuserdata = $this->likedislikeuserdata($without_extension, '1');
	...
	private function likedislikeuserdata($imgname, $status) {
		...
		$this->db->join('user_app', 'user_app.id = likeproductimg.userid', 'left');
		$this->db->where('likeproductimg.likestatus', $status);
		$this->db->where('likeproductimg.imgname', $imgname);
		$query = $this->db->get('likeproductimg');
		$data = array();
		$i = 0;
		if ($query->num_rows() > 0) {
			$result = $query->result_array();
