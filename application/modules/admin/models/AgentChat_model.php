<?php
class AgentChat_model extends CI_Model {
	...
	public function productsCount($search_title = null, $orderby = null, $category = null) {
		if ($search_title != null) {
			...
			$this->db->where("(agent.name LIKE '%$search_title%' OR agent.phone LIKE '%$search_title%' OR agent.email LIKE '%$search_title%'");
		}
		// if (isset($orderby) && $orderby !== null && $orderby != '') {
		// 	$this->db->where('agent.isverified', $orderby);
		// 	// $ord = explode('=', $orderby);
		// 	// if (isset($ord[0]) && isset($ord[1])) {
		// 	// 	$this->db->order_by('agent.' . $ord[0], $ord[1]);
		// 	// }
		// }
		// if ($category != null) {
	...
	public function getProducts($limit, $page, $search_title = null, $orderby = null, $category = null, $vendor = null) {
		if ($search_title != null) {
			$search_title = trim($this->db->escape_like_str($search_title));
			$this->db->where("(agent.name LIKE '%$search_title%' OR agent.phone LIKE '%$search_title%' OR agent.email LIKE '%$search_title%'");
			// $this->db->or_where("(agent.mobilenumber LIKE '%$search_title%')");
			// $this->db->or_where("(agent.emailid LIKE '%$search_title%')");
		}
		// if (isset($orderby) && $orderby !== null && $orderby != '') {
		// 	$this->db->where('agent.isverified', $orderby);
		// 	// $ord = explode('=', $orderby);
		...
		$this->db->order_by('readstatus', 'desc');
		$this->db->select("(SELECT COUNT(*) FROM wpn_chatmessenger1 WHERE wpn_chatmessenger1.read_status=0 AND wpn_chatmessenger1.usertype != 'admin' AND wpn_chatmessenger1.sender_id=agent.agent_id) AS readstatus,agent_id,name,phone,email", FALSE);
		$query = $this->db->get('agent');
		// $query = $this->db->select('id,name,mobilenumber,emailid,compnay,contactid,isverified')->get('agent', $limit, $page);
	...
	public function productStatusChange($id, $to_status) {
		$this->db->where('agent_id', $id);
		$result = $this->db->update('agent', array('visibility' => $to_status));
		return $result;
	}
	...
	public function setProduct($post, $id = 0) {
		...
		if ($id > 0) {
			$is_update = true;
			if (!$this->db->where('id', $id)->update('agent', array(
				'name' => $post['name'],
	...
	public function chatcomment($userid) {
		...
		$this->db->update('wpn_chatmessenger1', array('read_status' => 1));

		$this->db->where('userid', $userid);
		$this->db->join('agent', 'agent.agent_id = wpn_chatmessenger1.sender_id', 'left');
		//$this->db->orWhere('receiver_id',$userid);
		$this->db->order_by('wpn_chatmessenger1.id', 'desc');
		$this->db->where("(sender_id=" . $userid . " OR receiver_id=" . $userid . ")", NULL, FALSE);
		$query = $this->db->select('wpn_chatmessenger1.*,agent.name')->get('wpn_chatmessenger1');
