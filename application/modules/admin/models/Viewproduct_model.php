...
class Viewproduct_model extends CI_Model {
	...
	public function getProducts($limit, $page, $search_title = null, $orderby = null, $category = null, $vendor = null, $status = null) {
		...
		$this->db->where('userviewproduct.productid', $page);
		$this->db->join('user_app', 'user_app.id = userviewproduct.userid', 'left');
		$this->db->group_by('userviewproduct.userid');
		$query = $this->db->select('user_app.name,user_app.mobilenumber,COUNT(userviewproduct.userid) as viewcount')->get('userviewproduct');
		return $query->result();
	}
	...
	public function getproductinfo($id) {
		...
		$this->db->limit(1);
		$query = $this->db->select('products.*, products_translations.title, products_translations.description, products_translations.price, products_translations.old_price, products_translations.abbr, products.url, products_translations.for_id, products_translations.basic_description')->get('products');
