...
class User_model extends CI_Model {
	...
	public function getProducts($limit, $page, $search_title = null, $orderby = null, $category = null, $vendor = null) {
		...
		$query = $this->db->select('id,name,mobilenumber,emailid,contactid,isverified')->get('user_app', $limit, $page);
	...
	public function sendordersmsg($id) {
		$this->db->where('user_app.id', $id);
		$this->db->select('user_app.id,user_app.fcmtoken');
		$result1 = $this->db->get('user_app');
		// echo $this->db->last_query();
		$res = $result1->row_array();
		// echo '<pre>';
		// print_r($res);
		// exit;
		$message = $this->input->post("commentmsg");
		notifications('Ramrasiya Chat Message', $message, $res['fcmtoken']);
