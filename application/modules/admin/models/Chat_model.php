...
class Chat_model extends CI_Model {
	...
	public function getProducts($limit, $page, $search_title = null, $orderby = null, $category = null, $vendor = null) {
		...
		$this->db->select("(SELECT COUNT(*) FROM wpn_chatmessenger WHERE wpn_chatmessenger.read_status=0 AND wpn_chatmessenger.usertype != 'admin' AND wpn_chatmessenger.sender_id=user_app.id) AS readstatus,id,name,mobilenumber,emailid,compnay,contactid,isverified", FALSE);
		$query = $this->db->get('user_app');
		// $query = $this->db->select('id,name,mobilenumber,emailid,compnay,contactid,isverified')->get('user_app', $limit, $page);
