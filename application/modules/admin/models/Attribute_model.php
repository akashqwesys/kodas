...
class Attribute_model extends CI_Model
{
    ...
    {
        return $this->db->count_all_results('shop_attribute');
    }

    public function getShopAttribute($limit = null, $start = null)
    {
        $limit_sql = '';
	...
	public function getShopmainAttribute($limit = null, $start = null)
    {
        $limit_sql = '';
        if ($limit !== null && $start !== null) {
            $limit_sql = ' LIMIT ' . $start . ',' . $limit;
    ...
    public function deleteShopAttribute($id)
    {
        ...
        if (!$this->db->delete('shop_attribute_translations')) {
            log_message('error', print_r($this->db->error(), true));
        }

        $this->db->where('id', $id);
        $this->db->or_where('sub_for', $id);
        if (!$this->db->delete('shop_attribute')) {
            log_message('error', print_r($this->db->error(), true));
        }
    ...
    public function setShopAttribute($post)
    {
        $this->db->trans_begin();
        if (!$this->db->insert('shop_attribute', array('sub_for' => $post['sub_for'],'position' => $post['position']))) {
            log_message('error', print_r($this->db->error(), true));
        }
        ...
        foreach ($post['translations'] as $abbr) {
            ...
            $arr['for_id'] = $id;
            if (!$this->db->insert('shop_attribute_translations', $arr)) {
                log_message('error', print_r($this->db->error(), true));
            }
            $i++;
        }
    ...
    public function editShopAttributeSub($post)
    {
        $result = true;
        if ($post['editSubId'] != $post['newSubIs']) {
            $this->db->where('id', $post['editSubId']);
            if (!$this->db->update('shop_attribute', array(
    ...
    public function editShopAttribute($post)
    {
        $this->db->where('abbr', $post['abbr']);
        $this->db->where('for_id', $post['for_id']);
        if (!$this->db->update('shop_attribute_translations', array(
                    'name' => $post['name']
                ))) {
            log_message('error', print_r($this->db->error(), true));
            show_error(lang('database_error'));
    ...
    public function editShopAttributePosition($post)
    {
