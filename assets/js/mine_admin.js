...
//Edit Shop Attribute
var indicEditAttribute;
var forIdEditAttribute;
var abbrEditAttribute;
$('.editAttribute').click(function() {
    indicEditAttribute = $(this).data('indic');
    forIdEditAttribute = $(this).data('for-id');
    abbrEditAttribute = $(this).data('abbr');
    var position = $(this).position();
    $('#attributeEditor').css({ top: position.top, left: position.left, display: 'block' });
    $('#attributeEditor input').val($('#indic-' + indicEditAttribute).text());
});
$('.closeEditAttribute').click(function() {
    $('#attributeEditor').hide();
});

$('.saveEditAttribute').click(function() {
    $('#attributeEditor .noSaveEdit').hide();
    $('#attributeEditor .yesSaveEdit').css({ display: 'inline-block' });
    var newValueFromEdit = $('[name="new_value"]').val();
    $.ajax({
        type: "POST",
        url: urls.editShopAttribute,
        data: { for_id: forIdEditAttribute, abbr: abbrEditAttribute, type: 'shop_attribute', name: newValueFromEdit }
    }).done(function(data) {
        $('#attributeEditor .noSaveEdit').show();
        $('#attributeEditor .yesSaveEdit').hide();
        $('#attributeEditor').hide();
        $('#indic-' + indicEditAttribute).text(newValueFromEdit);
    });
});
...
$('.editAttributeSub').click(function() {
    ...
    var subForId = $(this).data('sub-for-id');
    $('[name="editSubId"]').val(subForId);
    $('#attributeSubEdit').css({ top: position.top, left: position.left, display: 'block' });
});
$('[name="newSubIs"]').change(function() {
    $('#attributeEditSubChanger').submit();
});
...
function mychatroom(id, name) {
    // alert(id);
    $.ajax({
        type: "POST",
        url: urls.chatfunction,
        data: { user_id: id }
    }).done(function(data) {
        var myObj = $.parseJSON(data);
        $('#msgdata').html(myObj.chatdata);
        $('#mycustommsgtitle').html('<i class="fa fa-info-circle"></i> Chat Message with ' + name);
        $('#receiver_id').val(myObj.userid);
    });
}

function mychatroomagent(id, name) {
    // alert(id);
    $.ajax({
        type: "POST",
        url: urls.chatfunctionagent,
        data: { agent_id: id }
    }).done(function(data) {
        var myObj = $.parseJSON(data);
        // alert(myObj.chatdata);
        $('#msgdata').html(myObj.chatdata);
        $('#mycustommsgtitle').html('<i class="fa fa-info-circle"></i> Chat Message with ' + name);
        $('#receiver_id').val(myObj.userid);
    });
}
$('.sendmsgtochatagent').click(function() {
    ...
    if (receiver_id != '') {
        $.ajax({
            type: "POST",
            url: urls.chatsendfunctionagent,
            data: { sender_id: sender_id, receiver_id: receiver_id, commentmsg: commentmsg }
        }).done(function(data) {
            var myObj = $.parseJSON(data);
            $('#sucessmsg').html(myObj.msg);
            $('#commentmsg').val('');
            setTimeout(function() {
                $('#sucessmsg').html('');
            }, 3000);
            mychatroomagent(myObj.userid);
        });
    } else {
        alert('Please First Select User');
    }
});



$('.sendmsgtochat').click(function() {
    var sender_id = $('#sender_id').val();
    var receiver_id = $('#receiver_id').val();
    var commentmsg = $('#commentmsg').val();
    if (receiver_id != '') {
...
//Pre products publish
function preremoveSecondaryProductImage(image, folder, container) {
    $.ajax({
        type: "POST",
        url: urls.preremoveSecondaryImage,
        data: { image: image, folder: folder }
    }).done(function(data) {
        $('#image-container-' + container).remove();
    });
...
// Upload More PDF on publish product
$('.finish-upload-pdf').click(function() {
    $('.finish-upload-pdf .finish-text-pdf').hide();
    $('.finish-upload-pdf .loadUploadpdffile').show();
    var someFormElement = document.getElementById('uploadPdfForm');
    var formData = new FormData(someFormElement);
    $.ajax({
        url: urls.uploadOthersPdf,
        type: "POST",
        data: formData,
        contentType: false,
        cache: false,
        ...
        success: function(data) {
            if (data != 'Invalid') {
                $('.finish-upload-pdf .finish-text-pdf').show();
                $('.finish-upload-pdf .loadUploadpdffile').hide();
                reloadOthersImagesContainer();
                $('#modalMorePdf').modal('hide');
...
$('.closePositionAttribute').click(function() {
    $('#positionEditor').hide();
});

$('.savePositionAttribute').click(function() {
    var new_val = $('[name="new_position"]').val();
    var editId = $('[name="positionEditId"]').val();
    $.ajax({
        type: "POST",
        url: urls.editPositionAttribute,
        data: { editid: editId, new_pos: new_val }
    }).done(function(data) {
        $('#positionEditor').hide();
        $('#position-' + editPositionField).text(new_val);
...
function mainorderstatus(id) {
    var statusname = $('#mainorderstatus').val();
    var orderid = id;
    $.post(urls.changeOrdersOrderStatus, { the_id: orderid, to_status: statusname }, function(data) {
        if (data == '1') {
...
function directorderstatus(id) {
    var statusname = $('#directorderstatus').val();
    var orderid = id;
    $.post(urls.changedirectOrderStatus, { the_id: orderid, to_status: statusname }, function(data) {
        if (data == '1') {
            alert('Status Updated Successfully!');
        } else {
            alert('Error with status change. Please check logs!');
